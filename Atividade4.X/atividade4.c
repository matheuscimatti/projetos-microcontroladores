...
void __interrupt(high_priority) atv4Timer(void){
    if(INTCONbits.TMR0IF && INTCONbits.TMR0IE){
        cursor_lcd_pos(0x85);//posiciona o cursor para o relogio ficar mais ou menos centralizado
        escrever_timer(min, seg);
        if(seg<59){
            seg++;
        }else{
            seg = 0;
        ...
        if(min>99){ //Caso exceder 99 minutos e 59s, reseta o relógio (não o pic)
            seg = 0;
            min = 0;
        }
        ...
        TMR0 = 3036;//timer inicial configurado para melhorar precisão
    ...
    if(INTCON3bits.INT1IE && INTCON3bits.INT1IF){
        ...
        wdtAux = 1;//altera a flag pra parar de limpar o temporizador de WDT
    }
}
...
void main(void) {
    ...
    T0CON = 0b10000011;//Timer0 com CLK interno, 16 bits
                       //e prescaler 1:16
    TMR0 = 3036;//Inicia o timer em 3036, para melhor precisão no resultado (1 segundo por clk)
